...
type TournamentEntity struct {
	CreatorUserID        string `json:",omitempty"`
	Status               string
	...
	ContestantsCount     int                 `datastore:",noindex,omitempty" json:",omitempty"`
	GamesCount           int                 `datastore:",noindex,omitempty" json:",omitempty"`
...
func IsMonthlyTournamentID(tournamentID string) bool {
	if len(tournamentID) == 6 {
		if v, err := strconv.ParseInt(tournamentID, 10, 32); err == nil && v > 201801 && v < 210000 {
			return true
		}
	}
	return false
